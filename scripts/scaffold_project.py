...
Examples:
  ...
  python scaffold_project.py --name customer-segments --task-type unsupervised \\
    ...
    parser.add_argument(
        "--output-dir", default="..", help="Parent directory for the new project (default: ..)"
    )
    parser.add_argument(
        "--task-type",
...
def validate_project_name(name: str) -> None:
    ...
    if any(c in name for c in invalid_chars):
        raise ValueError(
            f"Project name contains invalid characters. " f"Avoid: {' '.join(invalid_chars)}"
        )
...
def scaffold_project(args):
    ...
    # Templates
    if not render_template_safe(
        env, "environment.yml.template", context, target_dir / "environment.yml"
    ):
        failed_operations.append("Render environment.yml")

    if not render_template_safe(env, "setup.sh", context, target_dir / "setup.sh"):
